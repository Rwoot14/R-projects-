...
file_output <- "Home_4_Question_2_finished2.txt" 
sink (file_output, append=FALSE, split=TRUE)

# Read data

#file_data <- "STA372_Homework4_Question2.dat.txt"
file_data = "HW4Question2.R"

Sales_data_table <- read.table(file_data, header = F, sep="") 
...
# Regress log(A) against Time and compute fitted values for log(A) and A   (a)
...
# Construct figure to plot A vs. Time and in-sample fitted values 
...
#Plot the in-sample forecasts and At 
...
Time <- c(Sales_data_table$Time,41:44)
Time_sq <- Time^2
Seasonal <- c(Sales_data_table$Seasonal,0.837664, 0.850020, 1.091490, 1.220826) 
...
#Plot in-sample forcast for sales (c)
